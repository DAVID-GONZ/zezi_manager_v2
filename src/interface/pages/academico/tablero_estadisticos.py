    ...
    with ui.element("div").classes("panel-card"):
        with ui.element("div").classes("panel-header"):
            ...
            with ui.element("div"):
                ...
                ui.label("Promedio del grupo ponderado por el peso de cada categoría").classes(
                    "tablero-panel-subtitle"
                )
...
def _render_actividades(datos: dict) -> None:
    ...
    with ui.element("div").classes("panel-card"):
        ...
        if not acts:
            with ui.element("div").classes("tablero-empty"):
                ui.label("Sin actividades evaluativas registradas.").classes("tablero-empty-hint")
            return
    ...
    with ui.element("div").classes("panel-card"):
        with ui.element("div").classes("panel-header"):
            ...
            with ui.element("div"):
                ...
                ui.label(
                    f"{n_est} estudiantes · {n_act} actividades — ordenados por promedio ascendente"
                ).classes("tablero-panel-subtitle")
        ...
        if not heatmap_data:
            with ui.element("div").classes("tablero-empty"):
                ...
                ui.label("Sin notas registradas para mostrar el mapa de calor.").classes(
                    "tablero-empty-hint"
                )
            return
                ...
                ui.echart(
                    {
                        "tooltip": {
                            "position": "top",
                            "formatter": (
                                "params => params.value ? (params.value[2] + '/100') : 'Sin nota'"
                            ),
                        },
                        "grid": {
                            "left": "22%",
...
def _render_tendencia_asistencia(datos: dict) -> None:
    ...
    with ui.element("div").classes("panel-card"):
        ...
        if len(tendencia) < 2:
            with ui.element("div").classes("tablero-empty"):
                ui.label("Datos insuficientes para mostrar la tendencia.").classes(
                    "tablero-empty-hint"
                )
                ui.label("El tablero se actualiza con cada registro de asistencia.").classes(
                    "tablero-empty-hint"
                )
            return
    ...
    with ui.element("div").classes("panel-card"):
        with ui.element("div").classes("panel-header"):
            ...
            with ui.element("div"):
                ...
                ui.label(
                    "Ordenada por promedio ascendente — los estudiantes en riesgo aparecen primero"
                ).classes("tablero-panel-subtitle")
...
def _cargar_datos_globales(_s: dict) -> None:
    ...
    if not periodo_id:
        ...
        return
    try:
        metricas = Container.estadisticos_service().metricas_institucionales(periodo_id, anio_id)
    ...
    with ui.element("div").classes("panel-card"):
        with ui.element("div").classes("panel-header"):
            ...
            with ui.element("div"):
                ...
                ui.label("Nota definitiva media de todos los estudiantes").classes(
                    "tablero-panel-subtitle"
                )
                ...
                "series": [
                    {
                        ...
                        "data": [
                            ...
                            for p, c in zip(promedios, colores, strict=False)
                        ],
                        "barMaxWidth": 44,
                        "label": {
                            "show": True,
...
def tablero_estadisticos_page() -> None:
    ...
    def global_refreshable() -> None:
        ...
        if not _s["global_data"]:
            with ui.element("div").classes("tablero-empty"):
                ...
                ui.label("Aún no hay notas registradas en ningún grupo para este periodo.").classes(
                    "tablero-panel-subtitle"
                )
            return
    ...
    def drill_refreshable() -> None:
        ...
        if not _s["periodo_id"]:
            with ui.element("div").classes("tablero-empty"):
                ui.label("Selecciona un periodo para acceder al análisis detallado.").classes(
                    "tablero-empty-hint"
                )
            return
        ...
        if not _s.get("drill_grupo_id") or not _s.get("drill_asig_id"):
            with ui.element("div").classes("tablero-empty"):
                ...
                ui.label("Selecciona grupo y asignatura para ver el análisis detallado.").classes(
                    "tablero-empty-hint"
                )
            return
    ...
    def profesor_refreshable() -> None:
        ...
        if not _s.get("periodo_id") or not _s.get("grupo_id"):
            with ui.element("div").classes("tablero-empty panel-card"):
                ...
                ui.label("Usa el selector de periodo, grupo y asignatura.").classes(
                    "tablero-panel-subtitle"
                )
            return
        ...
        if datos.get("vacio") or datos.get("error"):
            with ui.element("div").classes("tablero-empty panel-card"):
                with ui.element("div").classes("tablero-empty-icon"):
                    ThemeManager.icono("bar_chart_off", size=40)
                ui.label(datos.get("error") or "Sin datos disponibles para este contexto.").classes(
                    "tablero-empty-hint"
                )
            return
                ...
                with ui.element("div").classes("panel-card"):
                    with ui.element("div").classes("panel-header"):
                        ...
                        with ui.element("div"):
                            ...
                            ui.label("Métricas globales del periodo — todos los grupos").classes(
                                "tablero-panel-subtitle"
                            )
                    global_refreshable()
